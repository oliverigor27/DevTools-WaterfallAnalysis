...
int main()
{
    ...
    while(1)
    {
        ...
        // Move os dados do Receiver queue do OS para o buffer da aplicação e lê esse buffer.
        read(client_fd, buffer, MAX_SIZE_BUFFER);
        printf("%s\n", buffer);

        // Simulla uma demora no processamento da requisição
        // Espera por 6 segundos e depois desbloqueia
        sleep(6);
